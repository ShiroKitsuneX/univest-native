  ...
  return (
    <BottomSheet visible={visible} onClose={onClose} T={T}>
      <View style={{ padding: 20, paddingBottom: 24 }}>
          ...
          {[1, 2].map(n => (
            <TouchableOpacity
              ...
              style={{
                ...
                paddingVertical: 6,
                borderRadius: 20,
                backgroundColor: ePick === n ? T.accent : T.card2,
                borderWidth: 1,
                borderColor: ePick === n ? T.accent : T.border,
              }}
            >
              <Text
                style={{
                  color: ePick === n ? AT : T.sub,
                  fontSize: 11,
                  fontWeight: '700',
                }}
              >
                {n}ª: {n === 1 ? eC1 || 'Escolher' : eC2 || 'Opcional'}
              </Text>
            </TouchableOpacity>
          ))}
          {(!!eC1 || !!eC2) && (
        ...
        >
          {(fbCourses.length ? fbCourses : ALL_COURSES)
            .filter(cc => cc.toLowerCase().includes(eSrch.toLowerCase()))
            .map(cc => {
              const s1 = eC1 === cc,
                s2 = eC2 === cc
              return (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (s1) {
                      setEC1('')
                      if (ePick === 1) setEpick(1)
                    } else if (s2) {
                      setEC2('')
                      if (ePick === 2) setEpick(2)
                    } else if (ePick === 1) {
                      setEC1(cc)
                      setEpick(2)
                    } else {
                ...
                >
                  ...
                  <Text
                    style={{ color: T.accent, fontSize: 11, fontWeight: '800' }}
                  >
                    {s1 && '1ª ✓'}
                    {s2 && '2ª ✓'}
                  </Text>
        ...
        <TouchableOpacity
          ...
          style={{
            padding: 14,
            borderRadius: 16,
            backgroundColor: eC1 ? T.accent : T.border,
            alignItems: 'center',
            marginTop: 12,
          }}
        >
