...
export function DiscoverCoursesModal({ visible, onClose, onPickCourse }) {
  ...
  useEffect(() => { if (!visible) setDarea(null); }, [visible]);
        ...
        {!dArea ? (
          ...
          <>
            ...
            {dArea.courses.map(cc => {
              ...
              return (
                <TouchableOpacity key={cc} onPress={()=>{ onPickCourse(cc); onClose(); }} style={{ ...cd(), padding:13, marginBottom:8, flexDirection:"row", alignItems:"center", gap:12 }}>
                  <View style={{ flex:1 }}>
