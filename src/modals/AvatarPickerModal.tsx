      ...
      <View style={{ padding: 20, paddingBottom: 24 }}>
          ...
          {AVATAR_PRESETS.map(e => (
            <TouchableOpacity
              ...
              style={{
                ...
                height: 52,
                borderRadius: 26,
                backgroundColor: tmpAv === e ? T.acBg : T.card2,
                borderWidth: 2,
                borderColor: tmpAv === e ? T.accent : T.border,
                alignItems: 'center',
                justifyContent: 'center',
              }}
            >
        ...
        >
          {AVATAR_COLORS.map(([c1c], idx) => (
            <TouchableOpacity
              key={idx}
              onPress={() => setTmpBgIdx(idx)}
              style={{
                width: 52,
                height: 52,
                borderRadius: 26,
                backgroundColor: c1c,
                borderWidth: tmpBgIdx === idx ? 3 : 1,
                borderColor: tmpBgIdx === idx ? '#fff' : c1c + '40',
              }}
            />
          ))}
        </View>
