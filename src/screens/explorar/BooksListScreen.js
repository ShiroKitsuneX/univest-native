  ...
  return (
    <View style={{ flex:1, backgroundColor:T.bg }}>
      <View style={{ flexDirection:"row", alignItems:"center", paddingHorizontal:20, paddingTop:insets.top+4, paddingBottom:10, borderBottomWidth:1, borderColor:T.border }}>
        <TouchableOpacity onPress={()=>{ setBooksSearch(""); onBack(); }} style={{ marginRight:12 }}>
                    ...
                    <View style={{ flexDirection:"row", alignItems:"center", padding:14, borderRadius:14, backgroundColor:isRead ? T.accent+"15" : isReading ? "#f59e0b15" : T.card2, borderWidth:1, borderColor:isRead ? T.accent+"40" : isReading ? "#f59e0b40" : T.border }}>
                      {showMenu ? (
                        <View style={{ flexDirection:"row", flex:1, gap:6 }}>
                          <TouchableOpacity onPress={(e) => { e.stopPropagation(); const newRead = {...readBooks}; delete newRead[item.id]; persistReadBooks(newRead); setBookMenu(null); }} style={{ flex:1, padding:10, borderRadius:10, backgroundColor:T.card, borderWidth:1, borderColor:T.border }}>
                            <Text style={{ color:T.muted, fontSize:12, fontWeight:"700", textAlign:"center" }}>○ Nenhum</Text>
                          </TouchableOpacity>
                          <TouchableOpacity onPress={(e) => { e.stopPropagation(); const newRead = {...readBooks, [item.id]: "reading"}; persistReadBooks(newRead); setBookMenu(null); }} style={{ flex:1, padding:10, borderRadius:10, backgroundColor:"#f59e0b30", borderWidth:1, borderColor:"#f59e0b" }}>
                            <Text style={{ color:"#f59e0b", fontSize:12, fontWeight:"700", textAlign:"center" }}>📖 Lendo</Text>
                          </TouchableOpacity>
                          <TouchableOpacity onPress={(e) => { e.stopPropagation(); const newRead = {...readBooks, [item.id]: "read"}; persistReadBooks(newRead); setBookMenu(null); }} style={{ flex:1, padding:10, borderRadius:10, backgroundColor:T.accent+"20", borderWidth:1, borderColor:T.accent }}>
