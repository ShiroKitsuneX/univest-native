    ...
    <ScrollView style={{ flex:1 }} contentContainerStyle={{ padding:16, paddingBottom:24 }}>
      <View style={{ ...cd(), overflow:"hidden", marginBottom:12 }}>
        ...
        <View style={{ alignItems:"center", marginTop:-40, paddingBottom:20, paddingHorizontal:22 }}>
          <TouchableOpacity onPress={onChangePhoto} style={{ width:80, height:80, borderRadius:40, backgroundColor:AVATAR_COLORS[avBgIdx][0], alignItems:"center", justifyContent:"center", borderWidth:4, borderColor:T.card }}>
          ...
          <View style={{ flexDirection:"row", gap:8, marginBottom:4, flexWrap:"wrap", justifyContent:"center" }}>
            {!!c1 && <TouchableOpacity onPress={onEditCourses} style={{ backgroundColor:T.acBg, paddingHorizontal:12, paddingVertical:4, borderRadius:20, borderWidth:1, borderColor:T.accent+"40" }}><Text style={{ color:T.accent, fontSize:11, fontWeight:"700" }}>1ª {c1}</Text></TouchableOpacity>}
            {!!c2 && <TouchableOpacity onPress={onEditCourses} style={{ backgroundColor:T.card2, paddingHorizontal:12, paddingVertical:4, borderRadius:20, borderWidth:1, borderColor:T.border }}><Text style={{ color:T.sub, fontSize:11, fontWeight:"700" }}>2ª {c2}</Text></TouchableOpacity>}
            <TouchableOpacity onPress={onEditCourses} style={{ paddingHorizontal:8, paddingVertical:4, borderRadius:16, backgroundColor:T.card2, borderWidth:1, borderColor:T.border }}>
          ...
          <View style={{ flexDirection:"row", justifyContent:"center", gap:0, width:"100%", borderTopWidth:1, borderColor:T.border, paddingTop:14 }}>
            {[{v:followedCount,l:"seguindo",isFollowing:true},{v:gs.filter(g=>g.type!=="simulado").length,l:"provas"},{v:gs.filter(g=>g.type==="simulado").length,l:"simulados"},{v:Object.values(saved).filter(Boolean).length,l:"salvos",isSaved:true}].map(({v,l,isFollowing,isSaved},i,arr)=>(
              <TouchableOpacity key={l} onPress={() => { if (isFollowing && v > 0) onShowFollowing(); else if (isSaved && v > 0) onShowSaved(); }} style={{ flex:1, alignItems:"center", borderRightWidth:i<arr.length-1?1:0, borderColor:T.border, paddingVertical:4 }}>
      ...
      <View style={{ backgroundColor:isDark?"#0a1f0d":"#f0fdf4", borderRadius:16, padding:14, borderWidth:1, borderColor:T.accent+"30", marginBottom:12 }}>
        ...
        {last ? (
          ...
          <TouchableOpacity onPress={goNotas} style={{ padding:9, borderRadius:12, backgroundColor:T.acBg, alignItems:"center", borderWidth:1, borderColor:T.accent+"40" }}>
      ...
      <View style={{ ...cd(), padding:15, marginBottom:12 }}>
        <View style={{ flexDirection:"row", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
          ...
          {Object.keys(readBooks).length > 0 && <TouchableOpacity onPress={onShowBooks} style={{ paddingHorizontal:10, paddingVertical:4, borderRadius:8, backgroundColor:T.card2, borderWidth:1, borderColor:T.border }}>
            <Text style={{ color:T.sub, fontSize:10, fontWeight:"700" }}>Ver todos</Text>
          </TouchableOpacity>}
        </View>
        {Object.keys(readBooks).length === 0 ? (
          <TouchableOpacity onPress={onShowBooks} style={{ flexDirection:"row", alignItems:"center", justifyContent:"center", gap:8, paddingVertical:12, backgroundColor:T.card2, borderRadius:12, borderWidth:1, borderColor:T.border }}>
      ...
      <View style={{ ...cd({ padding:15, marginBottom:12, backgroundColor:isDark?"#1a1a2e":"#f5f5ff" }), borderWidth:1, borderColor:T.accent+"30", borderRadius:16 }}>
        <View style={{ flexDirection:"row", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
          ...
          <TouchableOpacity onPress={onAddGoal} style={{ paddingHorizontal:10, paddingVertical:4, borderRadius:8, backgroundColor:T.acBg }}>
      ...
      <View style={cd({ padding:15 })}>
        ...
        {EVENTS.map((ev,i)=>(
          <TouchableOpacity key={ev.id} onPress={()=>onOpenEvent(ev)} style={{ flexDirection:"row", alignItems:"center", gap:10, paddingVertical:9, borderBottomWidth:i<EVENTS.length-1?1:0, borderColor:T.border }}>
      ...
      {goalsUnis.map(goal => {
        ...
        return (
          <View key={goal.id} style={{ marginBottom:10 }}>
            <TouchableOpacity onPress={()=>{const u=unis.find(x=>x.id===goal.id); if(u) onSelectUni(u);}} style={{ flexDirection:"row", alignItems:"center", backgroundColor:T.card2, borderRadius:12, padding:12, borderWidth:1, borderColor:T.border }}>
                    ...
                    <TouchableOpacity onPress={() => {
                      ...
                      {showMenu ? (
                        <View style={{ flexDirection:"row", gap:4 }}>
                          <TouchableOpacity onPress={(e) => { e.stopPropagation(); const newRead = {...readBooks}; delete newRead[todo.bookKey]; persistReadBooks(newRead); setBookMenu(null); }} style={{ flex:1, padding:4, borderRadius:6, backgroundColor:T.card, borderWidth:1, borderColor:T.border }}>
                            <Text style={{ color:T.muted, fontSize:9, fontWeight:"700", textAlign:"center" }}>○</Text>
                          </TouchableOpacity>
                          <TouchableOpacity onPress={(e) => { e.stopPropagation(); const newRead = {...readBooks, [todo.bookKey]: "reading"}; persistReadBooks(newRead); setBookMenu(null); }} style={{ flex:1, padding:4, borderRadius:6, backgroundColor:"#f59e0b30", borderWidth:1, borderColor:"#f59e0b" }}>
                            <Text style={{ color:"#f59e0b", fontSize:9, fontWeight:"700", textAlign:"center" }}>📖</Text>
                          </TouchableOpacity>
                          <TouchableOpacity onPress={(e) => { e.stopPropagation(); const newRead = {...readBooks, [todo.bookKey]: "read"}; persistReadBooks(newRead); setBookMenu(null); }} style={{ flex:1, padding:4, borderRadius:6, backgroundColor:T.accent+"20", borderWidth:1, borderColor:T.accent }}>
