...
- ✅ **G.b (path alias)** — `babel-plugin-module-resolver` wired in `babel.config.js` with `@` → `./src`. Codemod rewrote 212 relative imports across 48 files (all of `src/` + `App.js`) to the `@/…` form. No `from "./…"` / `from "../…"` anywhere in the app tree. Requires Metro cache clear (`expo start -c`) on first run after pulling.
- ✅ **G.6 (ESLint + Prettier config)** — `.eslintrc.json` (eslint:recommended + react + react-native + prettier) and `.prettierrc.json` (no-semi, single-quote, trailing-comma=es5) in place. Devdeps installed. **Not yet enforced:** the codebase currently uses semicolons + double-quotes, so a blanket `prettier --write` would be a massive diff — leave that as a deliberate follow-up when you're ready to eat the reformat churn.
- ⏭️ **G.1 (TypeScript migration)** — deferred: bottom-up rename of stores → services → components → screens; significant scope, best done after the ESLint/Prettier reformat lands.
- ⏭️ **G.5 (testing infra)** — deferred: Jest + `@testing-library/react-native` not installed; start with pure-module tests (`src/utils/*`, store reducers) once the toolchain is in.
...
| G — Polish (was D) | 🟡 partial | G.a (resilience) + G.c (perf) + G.d (deadcode) + G.b (path alias, 212 imports migrated) + G.6 (ESLint/Prettier config) done. G.1 (TS) + G.5 (Jest) + enforcing Prettier reformat deferred | Low per change |

Execution order: **C.1 → D → E → F → G**. D and E can interleave per-modal if you prefer smaller PRs. Remaining G items (TS migration, Jest tests, one-shot Prettier reformat) are independent follow-ups; pick them up when you have appetite for the churn each brings.
