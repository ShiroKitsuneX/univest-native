...
- ✅ **G.b (path alias)** — `babel-plugin-module-resolver` wired in `babel.config.js` with `@` → `./src`; mirrored in `tsconfig.json` `paths` and `jest.config.js` `moduleNameMapper`. Codemod rewrote 212 relative imports across 48 files (all of `src/` + `App.js`) to the `@/…` form. No `from "./…"` / `from "../…"` anywhere in the app tree. Requires Metro cache clear (`expo start -c`) on first run after pulling.
- ✅ **G.6 (ESLint + Prettier)** — `.eslintrc.json` (eslint:recommended + react + react-native + prettier) and `.prettierrc.json` (no-semi, single-quote, trailing-comma=es5) in place. **Prettier reformat applied across all `src/**` + `App.js`** — codebase now uses single quotes / no semicolons consistently.
- 🟡 **G.1 (TypeScript migration)** — `tsconfig.json` configured (`allowJs`, `noEmit`, `strict: false`, `moduleResolution: bundler` to match expo's base, `@/*` paths). Bottom-up file-by-file rename in progress:
  - ✅ **batch 1** — `src/utils/*` (4 files), `src/stores/*` + `middleware/persistToUser` (10 files), `src/services/*` (5 files). Each store gets a `State` interface; middleware typed as a zustand `StateCreator`; Firebase/Firestore payloads shaped at the service boundary. `tsc --noEmit` passes, jest 5/5.
  - ⏳ **batch 2** — `src/components/*` (7 files) + `src/theme/*`, `src/navigation/*`, `src/app/*`.
  - ⏳ **batch 3** — `src/modals/*` (13 files), `src/screens/**/*` (~12 files), `App.js`. Largest batch; expect prop-type work and event handler typing.
  - Once fully migrated, flip `strict: true` + `checkJs: true` and address the residual `any`/`unknown` leaks.
- ✅ **G.5 (test infra)** — `jest.config.js` uses `babel-jest` + `babel-preset-expo` (swapped away from `jest-expo` preset because it's pinned to expo 55 and we're on 54). `@/…` aliases resolved via `moduleNameMapper`. `testMatch: **/__tests__/**/*.test.js`. Seed test in `src/utils/__tests__/dates.test.js` (2 passing) proves the pipeline. Component tests with `@testing-library/react-native` will need a setup file once the first one is written.
- 🆕 **npm scripts** — `lint` / `format` / `format:check` / `test` / `test:watch` / `typecheck` added to `package.json`. CI can now wire these up directly.
...
| G — Polish (was D) | 🟡 partial | G.a (resilience) + G.c (perf) + G.d (deadcode) + G.b (path alias) + G.6 (ESLint/Prettier applied) + G.5 (Jest pipeline + seed test) done. G.1 (TS file-rename migration) is scaffolded but the bottom-up `.js → .ts` pass is still pending | Low per change |

Execution order: **C.1 → D → E → F → G**. D and E can interleave per-modal if you prefer smaller PRs. Remaining work: the G.1 `.js → .ts` bottom-up rename is the only migration-scoped item left; everything else is recurring hygiene (write tests, tighten lint rules, add CI gates).
