...
- 🟡 **G.1 (TypeScript scaffolding)** — `tsconfig.json` in place with `allowJs: true`, `checkJs: false`, `noEmit: true`, `strict: false`, and the `@/*` path mapping. Type-checker runs as an advisor (`npm run typecheck` = `tsc --noEmit`). Bottom-up file-by-file `.js → .ts` rename still pending; do it in batches (stores → services → components → screens) with `strict` flipped on only for migrated files via `// @ts-check` pragmas or a stricter nested `tsconfig`.
